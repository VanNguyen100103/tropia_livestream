#!/usr/bin/env pwsh
# =============================================================================
# dev-mobile.ps1 — flutter run (frontend_main) targeting a physical phone.
# =============================================================================
...
# embedded livestream module (lib/livestream) uses the right BACKEND_URL
# (the Go livestream backend on :3000). The main app's own backend is read
# from the bundled .env asset, so it isn't overridden here.
#
# Usage:
...
Write-Host "  Backend   : $backendUrl  (livestream module)" -ForegroundColor Green
...
try {
    ...
    Write-Warning "Cannot reach $backendUrl/health - is the Go livestream backend (cd backend; make dev-api) running?"
    Write-Warning $_.Exception.Message
}

# BACKEND_URL is consumed by lib/livestream/core/config/app_config.dart via
# String.fromEnvironment. The main app reads its own backend from the .env
# asset, so we only inject the livestream backend here.
$flutterArgs = @('run', '--dart-define', "BACKEND_URL=$backendUrl")
if ($Device) {
