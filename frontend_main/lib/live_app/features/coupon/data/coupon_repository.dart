...
class CouponRepository {
  ...
  /// GET /api/coupons/mine — voucher của shop mình (seller).
  Future<List<CouponModel>> listMine() async {
    final res = await _dio.get('/api/coupons/mine');
    final list = (res.data as Map<String, dynamic>)['data'] as List? ?? [];
    return list
        .map((e) => CouponModel.fromJson(e as Map<String, dynamic>))
        .toList();
  ...
  /// POST /api/coupons — seller tạo voucher cho shop của mình.
  Future<CouponModel> createShopVoucher({
    required String code,
    ...
    return CouponModel.fromJson(Map<String, dynamic>.from(res.data as Map));
  }
}
