...
class LiveRepository {
  ...
  /// Các buổi live đã kết thúc của CHÍNH user hiện tại đã có bản ghi (replay).
  /// GET /api/live/replays → { sessions: [...] }. Dùng cho tab "Live" trang cá
  /// nhân để host xem lại buổi phát của mình. Trả list rỗng nếu lỗi mạng /
  /// chưa từng live.
  Future<List<Map<String, dynamic>>> fetchMyReplays({
    int limit = 30,
    int offset = 0,
  }) async {
    try {
      ...
      AppLogger.logError(_tag, 'fetchMyReplays failed', e, null);
      return const [];
    }
  }
