...
import 'package:tropia_mobile_app_android/live_app/features/upload/data/upload_repository.dart';
import 'package:tropia_mobile_app_android/live_app/features/live/screens/live_setup_screen.dart';
import 'package:tropia_mobile_app_android/live_app/features/order/data/order_repository.dart';
import 'package:tropia_mobile_app_android/live_app/features/order/models/order_model.dart';
...
class _SellerDashboardScreenState extends State<SellerDashboardScreen>
  ...
  Future<void> _loadData() async {
    // Đảm bảo thông tin shop có sẵn (card + đổi logo) dù vào dashboard từ đường
    // nào — ShopProvider được chia sẻ trong liveScope nên load 1 lần là đủ.
    final sp = context.read<ShopProvider>();
    if (sp.myShop == null && !sp.isLoading) sp.loadMyShop();
    ...
    return Scaffold(
      ...
      body: NestedScrollView(
        headerSliverBuilder: (_, __) => [
          ...
          SliverToBoxAdapter(child: _buildVoucherEntry()),
        ],
        body: TabBarView(
          controller: _tabCtrl,
  ...
  Widget _buildShopCard(ShopModel? shop) {
    ...
    return Container(
      ...
      child: Row(
        children: [
          // Avatar shop — chạm để đổi ảnh: chọn ảnh → upload R2 → backend lưu
          // logo_url → thẻ/clip của shop hiện avatar mới.
          GestureDetector(
            onTap: _uploadingLogo ? null : () => _pickAndUploadLogo(shop),
            child: Stack(
  ...
  // ── Lối vào nhanh: Voucher của shop ───────────────────────────────────────
  // Card nổi bật để seller dễ tìm (trước đây chỉ có icon nhỏ trên AppBar). Tạo
  // voucher shop ở đây → thẻ video của shop hiện badge "Mua với Voucher".
  Widget _buildVoucherEntry() {
    return Padding(
      padding: const EdgeInsets.fromLTRB(
        AppSizes.md,
        0,
      ...
      child: InkWell(
        onTap: () => Navigator.of(context).push(
          MaterialPageRoute(builder: (_) => const SellerVoucherScreen()),
        ),
        borderRadius: BorderRadius.circular(AppSizes.radiusMd),
        child: Container(
          padding: const EdgeInsets.all(AppSizes.md),
              ...
              const Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 2),
                    Text(
                      'Tạo mã giảm giá — thẻ video sẽ hiện "Mua với Voucher"',
                      style: TextStyle(
  ...
  // ── Đổi avatar shop ───────────────────────────────────────────────────────
  // Chọn ảnh từ thư viện → upload lên R2 (backend tự lưu logo_url vào DB) →
  // cập nhật provider để UI + thẻ/clip của shop hiện ngay avatar mới.
  Future<void> _pickAndUploadLogo(ShopModel shop) async {
    ...
    AppLogger.logUserEvent(
      action: 'upload_shop_logo',
      context: _tag,
      metadata: {'shopId': shop.id},
    );
    try {
      final url = await UploadRepository.instance.uploadShopLogo(
        shop.id,
        File(picked.path),
      );
      if (!mounted) return;
      context.read<ShopProvider>().setLogo(url);
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Đã cập nhật ảnh đại diện cửa hàng')),
      );
    } catch (e, st) {
      AppLogger.logError(_tag, 'uploadShopLogo failed', e, st);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Không thể tải ảnh lên. Thử lại sau.')),
        );
      }
    } finally {
                ...
                onPressed: sp.isSaving
                    ...
                    : () async {
                        ...
                        if (!ok && mounted) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Text(sp.error ?? 'Lỗi khi cập nhật'),
                            ),
                          );
                        }
                      },
