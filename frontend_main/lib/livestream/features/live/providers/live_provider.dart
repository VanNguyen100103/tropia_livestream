...
import 'package:tropia_mobile_app_android/livestream/features/cart/data/cart_repository.dart';
...
import 'package:tropia_mobile_app_android/livestream/features/live/data/live_socket.dart';
import 'package:tropia_mobile_app_android/livestream/features/live/models/live_stream_model.dart';
import 'package:tropia_mobile_app_android/livestream/features/live/services/chat_command_parser.dart';
import 'package:tropia_mobile_app_android/livestream/features/live/services/ai_suggestion_service.dart';
...
class LiveProvider extends ChangeNotifier {
  ...
  Future<void> _loadStreams() async {
    ...
    try {
      ...
      if (_streams.isEmpty) {
        // Không có buổi live nào đang diễn ra là trạng thái BÌNH THƯỜNG, không
        // phải lỗi. Để errorMessage = null thì UI hiện màn "trống" thân thiện
        // thay vì màn lỗi (icon wifi gạch + "Thử lại"). Lỗi mạng thật vẫn được
        // bắt ở khối catch bên dưới.
        _errorMessage = null;
        AppLogger.logInfo(_tag, 'API returned no streams (empty, not an error)');
      } else {
  ...
  Future<void> refresh() async {
    ...
    try {
      ...
      // Rỗng = không có live, KHÔNG phải lỗi → để UI hiện màn trống.
      _errorMessage = null;
      await _saveStreamsToCache();
